...
write.file = "No"       # write out a file; don't use while troubleshooting
...
# adjust 'eggStage' for males; NA will break model code later
mean.topvels2$eggStage[mean.topvels2$sex=="m"] <- 99
...
```{r plots to evaluate study design}
  ...
  # look at speed and sex and egg stage
  ggplot(mean.topvels2, aes(x=factor(eggStage), y=nburstGoodFair)) + 
    geom_boxplot() + 
    geom_jitter(width=.15) +
    xlab("Egg Stage") + ylab("N 'Good' bursts")+
    facet_wrap(~sex, scales="free") + 
    theme_bw()   
  
  ggplot(mean.topvels2, aes(x=factor(eggStage), y=mnburstGoodFair_bls)) + 
    geom_boxplot()+
    geom_jitter(width=.15) +
    xlab("Egg Stage") + ylab("Est Burst Velocity (bl/s)")+
    facet_wrap(~sex, scales="free") + 
    theme_bw()   
  ...
  # Tank effects doesn't seem to be causing difference in growth or nburstGood, but are close to important for spd
  
  mean.topvels2$tagday[mean.topvels2$holdingtank %in% paste0("AES-",1:6)] <- 1   
  mean.topvels2$tagday[mean.topvels2$holdingtank %in% paste0("AES-",7:12)] <- 2
  summary(aov(lm(mnburstGoodFair_bls ~ tagday, data= mean.topvels2)))  
...
mainplot
  ...
  if(write.file=="Yes") {
    tiff("../figures/Bursts_Outputfigures_Expt1_DS2023/Burst_Expt1_DS2023.tiff", 
...
fullmod = lm(mnburstGoodFair_bls ~ treatment + nburstGoodFair + tunneltemp + fl_mm + triallength + sex*eggStage, data = filtered.moddat, na.action="na.fail" ) # could (should?) make this a lmer with holding tank as the random effect, but makes inference harder later
 ...
 dredge(fullmod)
   # best model with only treatment and tunnel temp (but none really good; weight here only 6.6%)
...
fullmod.participant= lm(nburstGoodFair ~ treatment + triallength + tunneltemp + fl_mm + sex + eggStage, data = filtered.moddat, na.action="na.fail")
...
dredge(fullmod.participant)  
 # best model only tunnel temp (and intercept) - weight = 15%
 # second best with only intercept (dAIC=0.72, weight = 10%) *most parsimonious within dAIC<2
