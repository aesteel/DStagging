...
trial_specs.7  <- read.csv("../rawData/Expt3_7day_DS2023_Burst_MetaTrialdat.csv")
trial_specs.30 <- read.csv("../rawData/Expt3_30day_DS2023_Burst_MetaTrialdat.csv")

trial_specs = rbind(trial_specs.7, trial_specs.30)


 # tidy up data types within the df
 trial_specs$trialdate = as.Date(trial_specs$trialdate, format="%m/%d/%y")#readr::parse_date(trial_specs$trialdate)
 trial_specs$tagdate =  as.Date(trial_specs$tagdate, format="%m/%d/%y")#readr::parse_date(trial_specs$tagdate)
 
 trial_specs$accstart = readr::parse_time(trial_specs$accstart, "%H:%M:%S")# %p")
 trial_specs$trialstart = readr::parse_time(trial_specs$trialstart, "%H:%M:%S")# %p")
 trial_specs$trialend = readr::parse_time(trial_specs$trialend, "%H:%M:%S")# %p")
...
# 115 trials, but removed trial T79 from dataset since fish LOE before bursts began
...
spp.yr.rawfilepath = "BurstExpt3_2023" # where the raw data files from raspPi are stored
spp.yr.figfolder = "Bursts_Outputfigures_Expt3_DS2023" # this folder should be created within the 'DS_Tagging/figures' folder before running this code
spp.yr.dataoutputfolder = "Bursts_Outputdata_Expt3_DS2023" # this folder should be created within the 'DS_Tagging/outputData' folder before running this code
...
#### need to do something here to correct for the fish burst after the middle laser was removed
## for now, subset to remove all trials run on 1/24/2024 so we can get a quick look at the first 3/4 of the data
## Note made Jan 30, 2024
mean.topvels2 = mean.topvels2[!(mean.topvels2$trialID %in% paste0("T",88:115)),] # 28 burst trials
...
# remove cold temp
mean.topvels2 = mean.topvels2[mean.topvels2$tunneltemp >12,] # 2 trials

# add column for 7d or 30d
mean.topvels2$testgroup[mean.topvels2$trialID %in% paste0("T",61:115)] <- "30d"
mean.topvels2$testgroup[!(mean.topvels2$trialID %in% paste0("T",61:115))] <- "7d"

# adjust data entry where spaces were added to excel file

mean.topvels2$treatment <- str_replace_all(mean.topvels2$treatment, " ", "")
mean.topvels2$sex <- str_replace_all(mean.topvels2$sex, " ", "")

write.csv(mean.topvels2, "../intermedData/Expt3_Burst_7d30d_compiledData.csv", row.names=F)
...
```{r plots to evaluate study design}
  ...
  ggplot(mean.topvels2, aes(x=as.numeric(str_sub(trialID,2,3)), y=mnburstGoodFair)) + 
    geom_point(aes(color=testgroup)) + 
  ...
  summary(aov(lm(mass_g ~ treatment + holdingtank + testgroup, data= mean.topvels2)))     
  summary(aov(lm(fl_mm ~ treatment + holdingtank + testgroup, data= mean.topvels2)))  
  summary(aov(lm(nburstGoodFair ~ treatment + holdingtank + testgroup, data= mean.topvels2)))  
  summary(aov(lm(mnburstGoodFair_bls ~ treatment + holdingtank + testgroup, data= mean.topvels2)))  
  summary(aov(lm(mnburstGoodFair_bls ~ treatment + holdingtank + mass_g + testgroup, data= mean.topvels2)))  
...
```{r plot effects on Good burst number}
  ...
  ggplot(mean.topvels2, aes(x=nburstGoodFair, y=mnburstGoodFair)) + 
    geom_point(aes(color=nburstGoodFair), size=3) + geom_smooth() + facet_wrap(~testgroup)

  ggplot(mean.topvels2, aes(x=nburstGoodFair, y=sdburstGoodFair)) + 
    geom_point(aes(color=nburstGoodFair), size=3) + geom_smooth() + facet_wrap(~testgroup)
  
      table(mean.topvels2$treatment, mean.topvels2$testgroup)
  ...
  # look at relationship between estimated burst speeds and number of good bursts, by treatment group
     ggplot(filter(mean.topvels2, nburstGoodFair>3),
      ...
      scale_color_manual(values=c("grey90","cyan3","darkorange2"), 
                            labels=c("Full Control","Lateral Tagging","Ventral Tagging"),
                            name="Treatment Group") + 
      facet_grid(testgroup~treatment) + theme_bw()
  ...
  theme_bw()
    ...
    # filter dataset so treatment groups are simliar in fl and number
     filt.mean.topvels2 = filter(mean.topvels2, fl_mm>65 & fl_mm<81)
        table(filt.mean.topvels2$treatment, filt.mean.topvels2$testgroup)
            #.      FC LT VT 
            #.  7d  19 16 17
            #. 30d   6  6  7 # without 24th data
        
        summarize(group_by(filt.mean.topvels2, treatment, testgroup), 
...
```{r report or paper plots}     
    ...
    # filter dataset so treatment groups are simliar in fl and number, and only 7d
     filt.mean.topvels2 = filter(mean.topvels2, fl_mm>65 & fl_mm<81 & testgroup=="7d")
        table(filt.mean.topvels2$treatment, filt.mean.topvels2$testgroup)
...
mainplot.filt = ggplot(filt.mean.topvels2, #filter(mean.topvels2, nburstGoodFair>3), 
                  aes(y=mnburstGoodFair_bls, x=as.numeric(as.factor(treatment))+random.offset)) + 
