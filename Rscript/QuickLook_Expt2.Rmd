...
metadat = read.csv("/Volumes/GoogleDrive/My Drive/2022 Delta Smelt Tagging/DataCollection/DStagging_Expt2_DStemp2023/Expt2_DStemp_TaggedFishMetadata.csv")
...
morts = read.csv("/Volumes/GoogleDrive/My Drive/2022 Delta Smelt Tagging/DataCollection/DStagging_Expt2_DStemp2023/MortNecropsy_DSExpt2_Temp.csv")
 # add leading 0 back into TagID_Hex
 morts$TagID_Hex = str_pad(morts$TagID_Hex,4, pad="0")
 
 # add mort date to tagdat NOT WORKING YET (8/3/2023)
 fishdat3 = merge(fishdat2, morts[,c("TagID_Hex","ExptMort","MortDate",
...
tagdat$RecovTime = as.numeric( difftime( as.POSIXct(paste(tagdat$DateTagged, tagdat$Time_recovered), format="%d-%b-%y %H:%M:%S", tz="America/Los_Angeles"), as.POSIXct(paste(tagdat$DateTagged, tagdat$Time_out_surgery), format="%d-%b-%y %H:%M:%S", tz="America/Los_Angeles") ) )
```
...
mort.order = ggplot(tagdat, 
                     aes(y=(ExptMort), x=OrderID, color=Sex) ) + 
  geom_point(size=3) + 
